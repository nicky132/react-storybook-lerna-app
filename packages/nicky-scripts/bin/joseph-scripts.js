...
const scriptIndex = args.findIndex((x) => x === 'localize');
const script = scriptIndex === -1 ? args[0] : args[scriptIndex];
const nodeArgs = scriptIndex > 0 ? args.slice(0, scriptIndex) : [];

if (['initialize', 'localize'].includes(script)) {
    const result = spawn.sync(process.execPath, nodeArgs.concat(require.resolve('../scripts/' + script)).concat(args), { stdio: 'inherit' });
    if (result.signal) {
    ...
    console.log('Perhaps you need to update nexus-scripts?');
    console.log('See to find some update');
}
